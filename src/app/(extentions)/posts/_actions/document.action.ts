...
import * as postsQuery from "./posts.query"; // 게시판 정보 조회를 위해 필요
...
import * as documentQuery from "@extentions/posts/_actions/document.query";
...
export const saveDocument = async (pid: string, formData: FormData, paths?: string): Promise<ActionState<number>> => {
  try {
    ...
    if (data.tempId) {
      await commitAttachments(data.tempId, resultId);
    }
    if (paths) {
...
// ==========================================
export async function getDocumentList(pid: string, page: number = 1, pageSize: number = 10, categoryId?: string): Promise<ActionState<any>> {
  try {
    const postInfo = await postsQuery.findPostsByPid(pid);
...
// ==========================================
export async function removeDocument(id: number, pid: string): Promise<ActionState<null>> {
  try {
    const loggedInfo = await getLoggedInfo();
    if (!loggedInfo) return { success: false, type: "error", message: "로그인이 필요합니다." };
