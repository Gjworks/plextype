...
import { usePostContext } from "@extentions/posts/_tpl/default/PostProvider";
import { CommentWithChildren } from "@extentions/posts/_actions/_type";
...
interface CommentsListProps {
  ...
  upsertComment: (args: {
    ...
    commentId?: number;
    options?: { deleted?: boolean; remove?: boolean };
  }) => Promise<{ success: boolean; item?: CommentWithChildren; actionType: string }>;
...
export default function CommentsList({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // ✅ 서버 응답이 성공이고 아이템이 있을 때만 상태 업데이트
      if (result.success && result.item) {
        setComments(prev => ({
          ...prev,
          items: [result.item!, ...prev.items], // 새 댓글을 맨 위로
          pagination: {
            ...prev.pagination,
        ...
        highlightComment(result.item.id);
        setNewContent("");
      }
    } finally {
      setLoading(false);
    ...
    try {
      ...
      if (result.success && result.item) {
        if (result.actionType === 'CREATE') {
          // 답글 등록 시: 기존 트리 구조 유지하며 merge
          setComments(prev => ({
            ...prev,
            items: mergeComments(prev.items, [result.item!]),
            pagination: { ...prev.pagination, totalCount: prev.pagination.totalCount + 1 }
          }));
          highlightComment(result.item.id);
        } else {
          // 수정 시: 리스트에서 해당 아이템만 교체
          setComments(prev => ({
            ...prev,
            items: prev.items.map(it => it.id === result.item?.id ? result.item : it)
          }));
        }
      }
